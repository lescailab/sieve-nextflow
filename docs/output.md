...
- When plot collection runs, `SIEVE_COLLECT_PLOTS` copies supported image files into `validation/plots/`, prefixes each copied filename with a sequence number and records its source in `plots_manifest.tsv`. If no plots are available, the directory contains `0000__no_plots_found.txt`.
