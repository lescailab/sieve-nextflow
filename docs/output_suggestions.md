...
4. **Resource optimisation**: If L1 performs nearly as well as L3, you might save computational resources
```
...
- Explainability visualisations (gene rankings, variant impacts)
...
- `train/`
  - Null model training artefacts (uses same hyperparameters as best model)
- `explain/`
...
- **Expected behaviour**: Null model should perform near-random (AUC ≈ 0.5); high attributions in null suggest overfitting or data leakage
...
| `null_model`                | `null/model/`                      | Null baseline training artefacts                  |
...
| `plots`                     | `plots/`                           | All publication-ready visualisations              |
| `pipeline_versions`         | `pipeline_info/`                   | Software versions and execution metadata          |

### Programmatic Access:

When running via Seqera Platform or with `-with-tower`, these outputs are published as named artefacts that can be:
...
- **Cause**: Step may have been skipped due to `--execute_step` parameter or upstream artefact injection
...
- **Solution**: Normal behaviour when running with `-stub-run` or when plot generation is disabled
